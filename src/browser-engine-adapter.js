...
class BrowserEngineAdapter {
  constructor(config, log = console.log) {
    ...
    this.lastExit = null;
    this.stopping = false;
    this.client = new InternalHttpClient({ baseUrl:config.internalBaseUrl, token:config.token, timeoutMs:30_000 });
  }

  status() {
    return {
      schema:'alps.v10200.browserEngineAdapter.v1',
  ...
  async start() {
    ...
    if (!fs.existsSync(this.config.legacyRunnerPath)) throw new Error(`Legacy browser engine adapter missing: ${this.config.legacyRunnerPath}`);
    this.stopping = false;
    this.startedAt = iso();
    ...
    const detectedAppDir = process.env.ALPS_APP_DIR || (
      ...
      fs.existsSync(path.join(localAppDir, 'index.html')) ? localAppDir : parentAppDir
    );
    const env = {
      ...process.env,
      ...
      // Preserve the filesystem semantics the browser runner had before isolation.
    ...
    this.child.on('exit', (code, signal) => {
      ...
      if (!this.stopping) {
        ...
        setTimeout(() => this.start().catch(error => this.log('[browser-engine] restart failed', summarizeError(error))), delay).unref();
      }
    });
    return this.status();
  }
  ...
  async waitUntilReady() {
    ...
    while (Date.now() < deadline) {
      last = await this.client.get('/runner/version', { timeoutMs:5_000 });
      if (last.ok && last.data && last.data.runtimeBootReady) return last.data;
      await new Promise(resolve => setTimeout(resolve, 1_500));
    }
    throw new Error(`Browser engine adapter did not become ready within ${this.config.childStartTimeoutMs}ms; last=${JSON.stringify(last && (last.error || last.data || last.status))}`);
  }
