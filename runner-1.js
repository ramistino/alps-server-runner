  ...
  try {
    return await pageEval(async cfg => {
      ...
      async function collectAllCandles(){
        let groups=[];
        try {
          // v10.1.7 candle visibility bridge: server-fetched real candles (data-api.binance.vision -> OKX -> Bybit chain)
          // are injected by the runner into __ALPS_V1017_SERVER_CANDLES__ and checked FIRST. Real closed candles only.
          if (globalThis.__ALPS_V1017_SERVER_CANDLES__) groups.push(...groupsFromContainer(globalThis.__ALPS_V1017_SERVER_CANDLES__, 'v1017ServerCandles'));
...
// v10.1.7 Feature Rows Materializer / Candle Visibility Bridge:
// When candles exist (server-fetched or page-held) but featureRowsFound = 0 / candlesVisibleToDiscovery = false,
// this bridge injects REAL server-fetched closed candles into __ALPS_V1017_SERVER_CANDLES__ inside the page,
// then re-runs the existing v951 feature/discovery engine so features, strategies, and candidates are built
// from real OHLCV rows through the existing real candidate path. No synthetic candles, candidates, trades, or OOS.
let lastV1017FeatureMaterializerView = null;
async function v1017FeatureMaterializerBridge(reason = 'v1017-feature-materializer') {
  const out = {
    ...
    realCandleRowsCollected: 0,
    canonicalPairFrames: 0,
    closedCandlePairFrames: 0,
    featureRowsBuilt: 0,
    discoveryRowsAfter: 0,
    candidateRowsAfter: 0,
    sourcesUsed: [],
    skippedReason: '',
    status: 'INIT',
    rule: 'Bridge real server-fetched closed candles into the page structures the existing v951 discovery/feature builder reads. Honest statuses only; never fabricate rows.'
  };
  try {
    if (!page || page.isClosed()) { out.status = 'SKIPPED'; out.skippedReason = 'PAGE_NOT_READY'; lastV1017FeatureMaterializerView = out; return out; }
    // 1) Collect real candles from the server bootstrap cache (filled by the binance.vision -> OKX -> Bybit chain).
    const groups = [];
    for (const [key, entry] of v1012ServerCandleCache.entries()) {
      const rows = safeArray(entry?.rows).filter(r => r && Number.isFinite(Number(r.open)) && Number.isFinite(Number(r.close)) && Number(r.t) > 0);
      if (rows.length < 120) continue;
      const parts = String(key).split('_');
      const pair = textValue(parts[0]).toUpperCase();
      const timeframe = textValue(parts[1]).toLowerCase();
      if (!pair || !timeframe) continue;
      groups.push({ pair, timeframe, sourceName: entry.sourceName || 'SERVER', rows: rows.map(r => ({ t: Number(r.t), open: Number(r.open), high: Number(r.high), low: Number(r.low), close: Number(r.close), volume: Number(r.volume || 0) })) });
      if (!out.sourcesUsed.includes(entry.sourceName || 'SERVER')) out.sourcesUsed.push(entry.sourceName || 'SERVER');
    }
    out.realCandleRowsCollected = groups.reduce((s, g) => s + g.rows.length, 0);
    out.canonicalPairFrames = groups.length;
    if (!groups.length) {
      // Trigger the bootstrap fetch chain once if the cache is empty, then honestly report if nothing real exists.
      const boot = await v1012ServerCandleResearchBootstrap(lastReport || {}, `${reason}-cache-fill`).catch(() => null);
      if (boot && safeArray(boot.candleGroups).some(g => n(g.rows, 0) >= 120)) return v1017FeatureMaterializerBridge(`${reason}-after-bootstrap`);
      out.status = 'NO_REAL_CANDLES_AVAILABLE';
      out.skippedReason = 'SERVER_CANDLE_CACHE_EMPTY_AND_BOOTSTRAP_RETURNED_NO_ROWS';
      lastV1017FeatureMaterializerView = out; return out;
    }
    // 2) Inject the real candle groups into the page global that collectAllCandles reads FIRST.
    const injected = await pageEval((serverGroups) => {
      try {
        const store = {};
        for (const g of serverGroups) { store[`${g.pair}_${g.timeframe}`] = g.rows; }
        globalThis.__ALPS_V1017_SERVER_CANDLES__ = store;
        return { ok: true, pairFrames: Object.keys(store).length };
      } catch (e) { return { ok: false, error: String(e && e.message || e).slice(0, 160) }; }
    }, groups).catch(e => ({ ok: false, error: e.message }));
    if (!injected?.ok) { out.status = 'CANDLES_FOUND_FEATURE_BUILDER_NOT_CONNECTED'; out.skippedReason = `PAGE_INJECTION_FAILED:${textValue(injected?.error)}`; lastV1017FeatureMaterializerView = out; return out; }
    out.closedCandlePairFrames = n(injected.pairFrames, 0);
    // 3) Re-run the existing v951 real-candle discovery so the page feature engine builds features + rows from the injected candles.
    const v951 = await v951CollectRealCandleDiscoveryMaterializer(`${reason}-post-injection`).catch(() => null);
    out.featureRowsBuilt = n(v951?.featureRowsFound, 0);
    out.discoveryRowsAfter = safeArray(v951?.rows).length;
    if (!out.featureRowsBuilt && out.realCandleRowsCollected > 0) { out.status = 'CANDLES_FOUND_FEATURE_BUILDER_NOT_CONNECTED'; lastV1017FeatureMaterializerView = out; return out; }
    if (out.featureRowsBuilt > 0 && !out.discoveryRowsAfter) { out.status = 'FEATURES_BUILT_DISCOVERY_ZERO_ROWS'; lastV1017FeatureMaterializerView = out; return out; }
    // 4) Commit discovered rows through the existing real candidate path (State Authority -> native pool -> latch).
    if (out.discoveryRowsAfter > 0) {
      try { v1000CommitRows(safeArray(v951.rows), 'v1017-feature-materializer'); } catch (_) {}
      const activeRows = safeArray(v1000ActiveRows());
      lastNativeForwardPoolView = v952BuildNativePoolFromRows(activeRows.length ? activeRows : safeArray(v951.rows), lastNativeForwardPoolView || {});
      lastForwardLatchView = v944BuildForwardLatchView();
      out.candidateRowsAfter = n(lastNativeForwardPoolView?.totalCandidates, 0);
      out.status = 'REAL_ROWS_MATERIALIZED_AND_COMMITTED';
    }
    lastV1017FeatureMaterializerView = out; return out;
  } catch (e) {
    out.status = 'FAILED'; out.skippedReason = textValue(e && e.message || e).slice(0, 180);
    lastV1017FeatureMaterializerView = out; return out;
  }
}
...
function v1015ShouldBootstrapMarketDataFromHealth(health = {}) {
  ...
  // v10.1.7 gate fix: the bootstrap exists precisely for the case where the browser pipeline is blind,
  // so browser-side candle visibility must never be a precondition for running it. Retry whenever the
  // pipeline is empty and no previous bootstrap attempt actually produced rows (a zero-row attempt must
  // not permanently close the gate). Real data only — the fetch chain itself never fabricates rows.
  const previousBootstrapProducedRows = safeArray(lastV1012ServerCandleBootstrapView?.rows).length > 0;
  return !previousBootstrapProducedRows;
}
...
async function v1015HealthMarketDataBootstrap(healthTruth = {}, reason = 'health-endpoint-v1015-market-data-bootstrap') {
  ...
  // v10.1.7: after the bootstrap, bridge server-fetched real candles into the page feature engine so
  // features/strategies/candidates are built through the existing v951 path (candle visibility bridge).
  try {
    const materializer = await v1017FeatureMaterializerBridge('health-endpoint-v1017-feature-materializer');
    healthTruth.v1017FeatureMaterializer = materializer;
    if (n(materializer?.candidateRowsAfter, 0) > 0) {
      healthTruth.nativeForwardPool = lastNativeForwardPoolView || healthTruth.nativeForwardPool;
      healthTruth.forwardLatch = lastForwardLatchView || healthTruth.forwardLatch;
      proof.after.nativePoolRows = v952Num(lastNativeForwardPoolView?.totalCandidates);
      proof.after.latchRows = v952Num(lastForwardLatchView?.size);
      proof.after.authorityRows = safeArray(v1000ActiveRows()).length;
    }
  } catch (e) { healthTruth.v1017FeatureMaterializer = { schema:'alps.v1017FeatureMaterializer.view.v1', installed:true, status:'FAILED', skippedReason:textValue(e.message||e).slice(0,160) }; }
  return proof;
}
...
async function collectReport() {
  ...
  // v10.1.7 gate fix: browser candle visibility must not gate the server bootstrap — the bootstrap exists
  // for exactly the case where the browser pipeline is blind. Pipeline emptiness alone triggers it.
  if (!safeArray(lastDiscoveryOutputView?.rows).length && !safeArray(lastV1012ServerCandleBootstrapView?.rows).length) {
    ...
    // v10.1.7: bridge server-fetched real candles into the page feature engine on the report path too,
    // so features/strategies/candidates are built through the existing v951 discovery pipeline.
    try {
      report.v1017FeatureMaterializer = await v1017FeatureMaterializerBridge('collect-report-v1017-feature-materializer');
    } catch (e) {
      report.v1017FeatureMaterializer = { schema:'alps.v1017FeatureMaterializer.view.v1', installed:true, status:'FAILED', skippedReason:textValue(e.message||e).slice(0,160) };
    }
