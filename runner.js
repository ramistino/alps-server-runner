...
const RECOVERY_PATCH_VERSION = 'v10.1.7c-paper-entry-authority-bridge-batch-scan';
...
const COGNITION_PATCH_VERSION = 'v10.1.7c-paper-entry-authority-bridge-batch-scan';
...
const AUTONOMY_PATCH_VERSION = 'v10.1.7c-paper-entry-authority-bridge-batch-scan';
...
const FINAL_V930_VERSION = 'v10.1.7c-paper-entry-authority-bridge-batch-scan';
...
async function v1017FeatureMaterializerCandleVisibilityBridge(healthTruth = {}, reason = 'health-endpoint-v1017-feature-materializer') {
  ...
  try {
    ...
    if (out.rows.length > 0) {
      v1000CommitRows(out.rows, 'v10.1.7c-paper-entry-authority-bridge-batch-scan', { observedRows: out.rows.length, featureRows: out.featureRows.length, closedCandlePairFrames: out.closedCandlePairFrames });
      v944MergeForwardLatch(out.rows, 'v10.1.7c-paper-entry-authority-bridge-batch-scan');
      const activeRows = safeArray(v1000ActiveRows());
...
async function v1017cPaperEntryAuthorityBridge(healthTruth = {}, reason = 'v1017c-paper-entry-authority-bridge', rowsOverride = null) {
  ...
  try {
    ...
    for (const c of scanRows) {
      ...
      // Server bridge proves a valid entry zone but does not create a standalone page trade ledger.
      // The Browser Paper Entry engine remains the only writer of persistent page openTrades.
      view.acceptedButDeferred += 1;
      addReject('VALID_ZONE_DEFERRED_TO_BROWSER_PAPER_LEDGER', c, { currentPrice: current, entry, stop, target, direction, serverAuthorityBridge: true, note: 'Valid server-side zone proof; persistent trade opening is deferred to Browser Paper Entry to avoid creating an unsynced paper ledger.' });
    }
